...
tags_vals = ["UNKNOWN", "O", "Name", "Degree", "Skills", "College Name", "Email Address","Phone number",
...
def train_and_val_model(
    ...
    for _ in trange(epochs, desc="Epoch"):
        ...
        training_acc.append(tr_accuracy)
...
def extract(path):
    df = pd.read_csv(path)
    comp_year=df['Graduation_Year']
    df['grad_year']=None
    experience=df['Companies_worked_at']
    df['experience']=None
    df['Intern']=None
    df['Currently Employed']=None

    pattern =r'([0-9]{4}|Present)' 
    present_year=['Present','present']
    intern =r'([0-9]{4}|Present|present|intern|INTERNSHIP|Intern)' 
    internship_names=['intern','INTERNSHIP','Intern']
    for row in range(0,len(df)):
        year=re.findall(pattern,str(comp_year[row]))
        exp=re.findall(pattern,str(experience[row]))
        print(row,year)
        print(row,exp)
        ...
        if(len(year)!=0):
            flag=True
            for i in year:
                if i in present_year:
                    flag=False
                    current_year = date.today() 
                    df.at[row,'grad_year']=current_year.year
            if flag:
                    df.loc[row,'grad_year']=max(year)
               # df['grad_year'][row]=max(year)
        else:
            df.loc[row,'grad_year']=None
        
        #Experience,Current_Employment
        if(len(exp)!=0):
            flag=True
            for i in exp:
                if i in present_year:
                    ...
                    current_year = date.today() 
                    start_date=min(exp)
                    end_date=current_year.year
                    df.at[row,'experience']=int(end_date) - int(start_date)
                    df.at[row,'Currently Employed']='Yes'
                else:
                    df.at[row,'Currently Employed']='No'
                
            if flag:
                    start_date=min(exp)
                    end_date=max(exp)
            #df['experience'][row]=int(end_date) - int(start_date)
                    df.at[row,'experience']=int(end_date) - int(start_date)
               # df['grad_year'][row]=max(year)
        else:
            df.loc[row,'experience']=None
        
        #Internship
        if(len(intern)!=0):
            flag=True
            for i in intern:
                if i in internship_names:
                    flag=False
                    df.at[row,'Intern']='Yes'
            if flag:
                df.at[row,'Intern']='No'
        else:
            df.loc[row,'Intern']=None
